...
parser.add_argument('--compare', "-co", action = "store_true")
...
compare_all_boolean = args.compare_all
...
def run(matl_set, matl_set_year, matl_set_number, compare_all_boolean, plvs):

    # Retrieve kinetic parameters for given material property set
    ...
    #csv_file_path       = "../Usefull tools" + '/' + matl_set + "_" + "dynamic_TGA_10K" + "_" + "FDS" + ".csv"
    #csv_file_path = "C:\Users\jcarl\OneDrive\Documents\Mac-fpLocal\matl-db\Verification\Usefull tools\MaCFP_PMMA_UMET_GP_dynamic_TGA_10K_FDS.csv"
    ...
    if compare_all_boolean is False:
        rms_err = plot_and_rms(total_mass,n_reactions,T_m,m_m,N,plvs)
        return rms_err
   # if compare_all_boolean is False:
   #     rms_err = plot_and_rms(matl_set, False, False,False, compare_all_boolean, total_mass,n_reactions,T_m,m_m,N,plt)
            
     
     
    #plot and determine rms error
    #fig = False
    #axs = False

def plot_and_rms(total_mass,n_reactions,T_m,m_m,N,plvs):
...
#axs = axs
    ...
    if compare_all_boolean is True:
      #  axs = 0
      #  fig = 0
...
#if compare_all_boolean is False:
#    matl_set_number = False
#    run(matl_set, matl_set_year, matl_set_number, compare_all_boolean)
...
if compare_all_boolean is True:
    ...
    axs = axs.flatten()
    for matl_set in matl_set_list:
            matl_set_number = matl_set_list.index(matl_set)
            matl_set_number = matl_set_list.index(matl_set)
            try:
                rms_err, axs, fig = run(matl_set, 2021, matl_set_number, compare_all_boolean)
            except(FileNotFoundError):
                print("error")
            print(f(matl_set,23)+ f(rms_err,2))
    plt.show()

matl_set_number = False
if plvs is True:
    try:
        run(matl_set, matl_set_year, matl_set_number, compare_all_boolean,plvs)
    except:
        pass
if plvs is False:
    run(matl_set, matl_set_year, matl_set_number, compare_all_boolean,plvs)
...
#run(matl_set, matl_set_year, False)
