

# Laboratory UMET - Unité de Matériaux et Transformation CNRS UMR 8207 Team ISP – Polymer Systems Engineering team R2Fire (Reaction to Fire and Fire Resistance)



This README file was prepared by Johan Sarazin. The experimental work was carried out by Anaïs Dhenin and Pierre Bachelet. For any questions or requests for additional information, please contact Johan Sarazin (johan.sarazin@centralelille.fr) or the head of the R2Fire research group, Serge Bourbigot (serge.bourbigot@centralelille.fr).



Prior to testing, all samples were conditioned in a controlled environment at room temperature (approximately 23 °C) and a relative humidity of approximately 50%.





## Experimental Conditions: Thermogravimetric Analysis (TGA)



UMET\_TGA\_N2\_10K

Instrument and furnace

type: Netzsch TG 209F1 Libra

Furnace : Ceramic
IDENTITY:  WOOD
PROJECT:   MaCFP-4 pyrolysis model calibration exercise  
Temperature Program

1 : 313K (with 3 vacuum purging)

2 : 313K isothermal (5min)

3 : 313K to 1073K 10K/min

4 : 1073K isothermal (5min)"
Initial sample moisture content (%)	NA
Sample geometry	: powdered  
Calibration type :

Temperature calibration were performed every 6 months using a set of 6 references materials : Melting Point Calibration :Metals: Pb, Sn, Bi, Zn, Al, Au

Before each experiment, a baseline test was performed using an empty crucible.
Crucible :

Type: Al2O3

Volume: 85 µl

No Lid  
Carrier gas and associated flow rate(s)

Type: Nitrogen

Flow rate: 40ml/min


| Test Name | O2 volume % | Initial Sample Mass (mg) |
|---|---:|---:|
| UMET_TGA_N2_10K_R1 | 0 | 4.0488 |
| UMET_TGA_N2_10K_R2 | 0 | 3.4049 |
| UMET_TGA_N2_10K_R3 | 0 | 4.2213 |



## Experimental Conditions: Thermogravimetric Analysis (TGA)



UMET\_TGA\_21%O2\_10K

Instrument and furnace

type: Netzsch TG 209F1 Libra

Furnace : Ceramic
IDENTITY:  WOOD
PROJECT:   MaCFP-4 pyrolysis model calibration exercise  
Temperature Program

1 : 313K (with 3 vacuum purging)

2 : 313K isothermal (5min)

3 : 313K to 1073K 10K/min

4 : 1073K isothermal (5min)"

Initial sample moisture content (%)	NA
Sample geometry	: powdered  
Calibration type :

Temperature calibration were performed every 6 months using a set of 6 references materials : Melting Point Calibration :Metals: Pb, Sn, Bi, Zn, Al, Au

Before each experiment, a baseline test was performed using an empty crucible.
Crucible :

Type: Al2O3

Volume: 85 µl

No Lid  
Carrier gas and associated flow rate(s)

Type: Nitrogen + Oxygen

Flow rate: 100ml/min


| Test Name | O2 volume % | Initial Sample Mass (mg) |
|---|---:|---:|
| UMET_TGA_21%O2_10K_R1 | 0 | 2.3484 |
| UMET_TGA_21%O2_10K_R2 | 0 | 4.0253 |
| UMET_TGA_21%O2_10K_R3 | 0 | 4.7632 |



## Experimental Conditions: TGA/DSC



UMET\_TGASTA\_N2\_3K / UMET\_DCSTA\_N2\_3K

Instrument and furnace

type: Netzsch STA 449 F1 Jupiter

Furnace : Pt
IDENTITY:  WOOD
PROJECT:   MaCFP-4 pyrolysis model calibration exercise  
Temperature Program

1 : 303K (with 3 vacuum purging)

2 : 303K isothermal (5min)

3 : 303K to 1173K 3K/min, Period 60s, amplitude 0,5K

4 : 1173K isothermal (5min)"

Initial sample moisture content (%)	NA
Sample geometry	: powdered  
Calibration type :

Temperature calibration were performed every 6 months using a set of 4 references materials : Biphenyl, Benzoic Acid, Cesium Chlorure, Baryum Carbonate

Before each experiment, a baseline test was performed using an empty crucible.
Crucible :

Type: Pt

Volume: 85 µl

with Lid  
Carrier gas and associated flow rate(s)

Type: Nitrogen

Flow rate: 60ml/min


conventional system endo up
| Test Name | O2 volume % | Initial Sample Mass (mg) |
|---|---:|---:|
| UMET_TGASTA_N2_3K_R1 | 0 | 3.0439 |
| UMET_TGASTA_N2_3K_R2 | 0 | 4.1092 |
| UMET_TGASTA_N2_3K_R3 | 0 | 2.8869 |


## Experimental Conditions: DSC



UMET\_DSC\_N2\_3K



Instrument and furnace

type: Discovery DSC
IDENTITY:  WOOD
PROJECT:   MaCFP-4 pyrolysis model calibration exercise  
Temperature Program

1 : Equilibrate 20°C
2 : Isothermal 10min
3 : Ramp 3K/min to 473.15K  Modulate Temperature Amplitude 1°C period 60,0s
3 : Isothermal 10min
4 : Ramp 3K/min to 273.15K  Modulate Temperature Amplitude 1°C period 60,0s
5 : Isothermal 10min
6 : Ramp 3K/min to 473.15K  Modulate Temperature Amplitude 1°C period 60,0s
7 : Isothermal 10min
8 : Ramp 3K/min to 313.15K  Modulate Temperature Amplitude 1°C period 60,0s

Initial sample moisture content (%)	NA
Sample geometry	: powdered  
Calibration type :

Temperature calibration were performed every 6 months using a set of references materials : Tzero, Indium, Sapphire
Crucible :

Type: Al pan

Volume: 85 µl

with Lid  
Carrier gas and associated flow rate(s)

Type: Nitrogen

Flow rate: 350ml/min (purge)



conventional system endo up
| Test Name | O2 volume % | Initial Sample Mass (mg) |
|---|---:|---:|
| UMET_DSC_N2_3K_R1 | 0 | 2.1300 |
| UMET_DSC_N2_3K_R2 | 0 | 3.9700 |
| UMET_DSC_N2_3K_R3 | 0 | 5.3900 |



## Experimental Conditions: Transient Plane Source (TPS) Conductivity



UMET\_Conductivity\_room\_temperature

IDENTITY:WOOD
PROJECT:  MaCFP-4 pyrolysis model calibration exercise
Temperature Program	20°C
Initial sample moisture content (%): NA
Samples were conditioned in a controlled environment at room temperature (approximately 23 °C) and a relative humidity of approximately 50%.
SAMPLE MASS /mg:            	NA
Sample geometry	Solid
Calibration type, materials used, and frequency	NA
INSTRUMENT:Hot Disk (TPS method)
Calibration type, materials used, and frequency	NA
Carrier gas and associated flow rate(s)	NA
Probe	Kapton 8563, Radius 9,868mm

###### Test Conditions/Results

| Test Name | Temperature K | Measurement Time s | Thermal Conductivity W/mK | Thermal Diffusivity mm2/s | Volumetric Specific Heat MJ/m3K |
|---|---:|---:|---:|---:|---:|
| Wood_TPS_Sample1 | 303.15 | 160 | 0.147 | 0.407 | 0.362 |
| Wood_TPS_Sample2 | 303.15 | 160 | 0.144 | 0.352 | 0.410 |


## Experimental Conditions: Diffusivity (LFA)



UMET\_Diffusivity\_room\_temperature

IDENTITY:WOOD
PROJECT:  MaCFP-4 pyrolysis model calibration exercise
Temperature Program	20°C
Initial sample moisture content (%): NA
Samples were conditioned in a controlled environment at room temperature (approximately 23 °C) and a relative humidity of approximately 50%.
SAMPLE MASS /mg: NA
Sample geometry	Solid size : 25.4mm\*24.4mm thickness 0.38mm
Calibration type, materials used, and frequency	NA
INSTRUMENT:LFA 467 hyperFlash Netzsch
Calibration type, materials used, and frequency	NA
Carrier gas and associated flow rate(s)	20ml/min (N2)


| Test Name | Th. diffusivity (mm²/s) | Result variability (±) (mm²/s) |
|---|---:|---:|
| Wood sample 1 | 0.195 | 0.005 |
| Wood sample 2 | 0.195 | 0.005 |



## Experimental Conditions: Mass Loss Cone



UMET\_MLC\_30kW



IDENTITY:WOOD
PROJECT:  MaCFP-4 pyrolysis model calibration exercise
Test Standard: ISO 13927

Extraction flow rate: NA

Sample Surface Area: 0.00884 m2

Square pan: 106mm x 106mm x 25mm

Retainer frame: None

Retaining grid: None

Backing Insulation: Calcium silicate (1cm). Density = 1300 kg/m3. Thermal Conductivity 0.31W/m-K (at 473K), 0.33W/m-K (at 673K), 0.35 W/m-K (at 873K), 0.38 W/m-K (at 1073K)

Thermocouple location: None

Ignition Source: spark ignitor

Initial sample moisture content (%): NA
Samples were conditioned in a controlled environment at room temperature (approximately 23 °C) and a relative humidity of approximately 50%.
Sample geometry	Solid size : 100mm\*100mm thickness 3cm +/-0.1cm
Calibration type, materials used, and frequency	methane (0.5 kW, 1kW, 2kW, 3kW, 4KW) according standards (Calibration every day before testing)



Distance 35mm (heat source/sample)

Test Heating Conditions

| Test Name | Heat Flux (kW/m²) | Heater Temperature (K) | Thickness (cm) | t_ign (s) | t_flameout (s) | Initial Mass (g) |
|---|---:|---:|---:|---:|---:|---:|
| UMET_MLC_30KW_R1 | 30 | 778 | 2.56 | 25 | 2001 | 101.30 |
| UMET_MLC_30KW_R2 | 30 | 778 | 2.56 | 21 | 2105 | 83.61 |
| UMET_MLC_30KW_R3 | 30 | 778 | 2.56 | 15 | 2020 | 110.94 |
| UMET_MLC_60KW_R1 | 60 | 991 | 2.57 | 3 | 1899 | 84.67 |
| UMET_MLC_60KW_R2 | 60 | 991 | 2.57 | 5 | 1859 | 96.64 |
| UMET_MLC_60KW_R3 | 60 | 991 | 2.57 | 5 | 1854 | 85.16 |


