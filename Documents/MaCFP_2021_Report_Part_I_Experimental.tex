...
The measurement data and related analysis presented in this predecisional draft report should be considered as a preliminary summary of experimental work submitted to the 2021 MaCFP Condensed Phase Workshop. This summary is prepared for subject matter experts to provide critical review, for experimentalists to identify and correct minor errors in their submitted datasets (e.g., scaling or units issues; or mislabelling and/or submission of incorrect data), and to provide modelers who wish to use these measurements for pyrolysis model developmnt a suitable overview of the information available on the MaCFP Github Repository.
For the purposes of model development, it may be desirable to have a 'community average' of a particular dataset under given conditions (e.g., representative, average mass and mass loss rate curves from TGA tests or heat release rate measurements from cone calorimeter tests). However, as presented in Section~\ref{exp_results} of this document, defining such an average is not necessarily straightforward. One of the valuable outcomes of this excercise is the demonstration that test conditions, procedure, and initial calibration may have a meaningful impact on measurement results. For now, individual datasets are presented and average curves are calculated excluding clear outliers; however, further analysis as to why exactly these differences exist - e.g., are they simply stochastic or if they were due to, for example, calibration differences or subtle variations and how exactly the tests were conducted - is needed. 
Ultimately, average curves presented here represent the aggregate of data as received, some of which may require corrections by the original submitting institution (e.g., if a dataset was incorrectly labeled or submitted). A valuable outcome of this workshop Developing requirements for data set quality and establishing a data review committee
 may be to demonstrate that it may or may not be possible to simply cherry pick one type of experiment from this lab and another type from that lab, as modelers often do to develop their parameter sets. Simply averaging everything together may hide this. It shouldn't be up to just me to decide which tests do or do not belong in this averaging (excluding obvious outliers). This is something that the community, or at the very least the committee + all those who submitted data, should discuss and come to a conclusion on together. Such an analysis will come in the final version of this report months down the road.

As a result, formal conclusions are not presented here. 

In the final version of this report, further detail will be providing regarding:  . Additionally, whether a single 'community average' exists..; 

A second, related, report will then focus on: 
(1) Cataloguing current approaches used to parameterize pyrolysis models 
(2) what are these parameters and what are the key we're looking for, and 
(3) Assessing the impact of the variability of model parameters on material burning behavior and fire growth predictions
