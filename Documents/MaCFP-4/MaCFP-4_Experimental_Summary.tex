...
Felix Armbrust; Technische Universität Braunschweig (TUB) \\
Yuji Nakamura; Toyohashi University of Technology (TUT) \\
...
\label{optical_tests}
Measurements of spectrally-dependent sample reflectivity were determined using experimental apparatuses consisting of an FTIR spectrometer coupled with a detector and an integrating sphere. First, a reference measurement is performed in the sphere without a sample, using reference surfaces representing low and high reflectance, followed by measurement of the flux reflected by the sample. Sample reflectivity is then calculated from the corrected ratio between the sample and reference signals. Various combinations of spectrometers, detectors, and integrating spheres can be used for sample reflectivity measurements.

Here, three institutions provided reflectivity data for virgin wood samples, and two institutions also provided data for charred wood samples. The char samples were obtained from specimens previously exposed in CAPA or cone calorimeter experiments at known incident heat fluxes. The measurements included directional-hemispherical reflectivity and were performed using FTIR systems equipped with gold-coated integrating spheres; among the reported setups were Bruker Invenio spectrometers with Bruker A562-G integrating spheres and liquid-nitrogen-cooled MCT detection. Reflectivity measurements were corrected using low- and high-reflectance references, such as a light trap and a gold reference. All available char and virgin wood samples were measured with the grain oriented parallel to the measured surface where reported.

As wood samples are opaque, their transmissivities are assumed equal to 0 and their absorptivities are calculated as the complement of reflectivity to 1: $\alpha=1-\rho$. The measured spectral reflectivity was further used to calculate temperature-dependent total emissivity by weighting the reflectivity with the blackbody spectral distribution. More information about this experimental technique is available elsewhere~\cite{boulet2012characterization}.
