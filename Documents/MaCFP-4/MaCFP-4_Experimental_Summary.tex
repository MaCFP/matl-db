...
\begin{figure}[h!]
    \centering
    \includegraphics[width=5in]{../SCRIPT_FIGURES/TGA/TGA_Wood_N2_10K_Mass_marked_regions.pdf}
\caption{Normalized mass measured in TGA experiments conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min. The highlighted regions indicate the low-temperature mass loss associated with apparent initial moisture content (black), the variability in residual mass immediately after the main decomposition stage (blue), and the additional variability in residue yield observed at higher temperatures (red).}
    \label{Fig:TGA_N2_10K_marked}

\end{figure}

\paragraph{Dry-Mass Normalisation} The first source of interlaboratory variability is associated with the initial mass loss at relatively low temperatures. The apparent moisture contents derived from the individual datasets are reported in Table~\ref{tab:wood_tga_results} and range approximately from 0 to 9~\%. This variability is also evident in the enlarged low-temperature view shown in Figure~\ref{Fig:TGA_N2_10K_moisture}. These differences do not necessarily represent differences in the original material itself, but are strongly affected by sample conditioning and the experimental procedure prior to the main heating stage. Some institutions dried or otherwise conditioned the samples before testing, while others tested the material without additional drying. In some datasets, the initial part of the measurement was also removed before the data were submitted. Consequently, the apparent initial moisture content should be interpreted with caution when comparing results between laboratories.

\begin{figure}[h!]
    \centering
    \includegraphics[width=5in]{../SCRIPT_FIGURES/TGA/TGA_Wood_N2_10K_Mass_zoom1_marked.pdf}
    \caption{Enlarged view of the low-temperature mass-loss region for TGA experiments conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min. The dashed vertical line at 400~K indicates the temperature used to define the dry reference mass, $m_{400}$. Apparent moisture content is calculated relative to this mass as $1-m_{400}/m_0$.}
    \label{Fig:TGA_N2_10K_moisture}
...
In the present analysis, the mass measured at 400~K, $m_{400}$, was selected as the reference dry mass. At this temperature, the initial moisture-related mass loss had essentially been completed for all datasets, while the sample mass remained approximately constant and the main pyrolysis stage had not yet begun. As expected, mass values at temperatures below 400~K exceeded unity after rescaling, since they include the mass subsequently lost during drying. The rescaling substantially reduces the differences between datasets prior to the onset of pyrolysis. However, it does not eliminate the variability observed after the main decomposition stage, indicating that differences in the measured residue yield cannot be explained by initial moisture content alone. Nevertheless, dry-mass normalization slightly improves the consistency of the derived residue yields, as seen in Table~\ref{tab:wood_tga_results}. Across the investigated datasets, the correction increased the calculated residue yield by at most approximately 2 percentage points at 700~K and 1.3 percentage points at 950~K. The remaining variability in residue yield is examined in the following paragraphs.

\paragraph{Char Yield} Char (residue) yield is one of the key quantities derived from TGA measurements and is commonly used as an input parameter in pyrolysis models. However, its value depends on the temperature at which it is evaluated, and there is no unique temperature at which the remaining solid can unambiguously be considered as char. In the present analysis, residue yields were therefore evaluated at both 700~K, shortly after the main decomposition stage, and 950~K, close to the end of the temperature programme used by most participating laboratories. As shown in Table~\ref{tab:wood_tga_results}, residue yields at 700~K range from approximately 15 to 30~\% of the initial sample mass, whereas the corresponding values at 950~K range from approximately 7 to 20~\% for datasets extending to this temperature. For most datasets, the residue yield decreases by approximately 5.5--7 percentage points between 700 and 950~K. More pronounced decreases are observed for several participants, most notably Call', Buff', and `Mallard', for which the reduction reaches approximately 10--16 percentage points.

A gradual decrease in residue mass at elevated temperature is expected even under nominally inert conditions. By this stage, hemicellulose and cellulose have largely decomposed, while slower decomposition and further carbonisation of lignin-derived material and the resulting char can continue over a broad temperature range, explaining part of the difference between residue yields measured at 700 and 950~K.  However, the substantially larger mass losses observed in some datasets suggest an additional mechanism (secondary reaction), with oxidation of the residual char being a likely contributor.

The presence of a secondary reaction was further investigated by `Call', who performed additional tests specifically designed to assess the influence of residual oxygen. These measurements showed that increasing the purge-gas flow rate, evacuating the measurement cell before the test, and preventing backflow eliminated the secondary oxidation step (Figure~\ref{Fig:TGA_pressure}). This confirms that the pronounced high-temperature mass loss observed in some datasets can result from oxygen entering or remaining in the nominally inert test environment. When such a secondary reaction is observed, the presence of oxygen should therefore be carefully considered. Its possible sources include insufficient evacuation of the system, short purge times before measurement, leaks or permeable tubing, low purge-gas flow rates allowing backflow, or contamination of the purge gas.

\begin{figure}
	\centering
	\includegraphics[width=0.85\linewidth]{../SCRIPT_FIGURES/TGA/TGA_Pressure.pdf}
\caption{Normalized mass measured in TGA experiments conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min. The original dataset includes measurements at 100~kPa and 40~Pa, while the additional 100~kPa measurements were conducted after modifications to the experimental procedure aimed at suppressing residual oxygen and the associated secondary reaction (char oxidation).}
	\label{Fig:TGA_pressure}
\end{figure}

\paragraph{Reduced Pressure} The measurements by `Call' shown in Figure~\ref{Fig:TGA_pressure} also provide an opportunity to examine the effect of pressure. In addition to the atmospheric-pressure measurements discussed above, this institution performed tests at 40~Pa under the same nominal heating rate of 10~K/min. The reduced-pressure measurements show a substantially lower residue yield immediately after the main decomposition stage compared with the 100~kPa measurements, suggesting that pressure may influence the amount of solid residue formed during pyrolysis. However, interpretation of the original pressure comparison is complicated by the residual-oxygen effects identified above. The additional 100~kPa measurements performed with improved oxygen control demonstrate that the pronounced high-temperature mass loss in the original atmospheric-pressure dataset was not caused by pressure itself. Consequently, while the results suggest an effect of pressure on residue formation, the available data from a single institution are insufficient to isolate or quantify this effect independently of other experimental conditions.

\paragraph{Oxidative Atmosphere} TGA measurements were also conducted in oxidative atmospheres containing between 3 and 21~\% oxygen. These conditions were investigated less extensively than those in pure nitrogen because char oxidation was not a primary focus of MaCFP-4, although it will be considered in future workshops. Figure~\ref{Fig:TGA_O2-21_10K} shows measurements from 4 repeated tests conducted in mixtures of oxygen and nitrogen at approximately atmospheric oxygen concentration ($X_{\rm O_2}=0.20$--0.21) and a nominal heating rate of $\beta=10$~K/min. In these tests, sample mass loss occurs in two distinct stages, with approximately 70~\% of the initial sample mass lost by $T=600$~K and most of the remaining mass lost between approximately $T=750$~K and $T=775$~K as a result of char oxidation.
