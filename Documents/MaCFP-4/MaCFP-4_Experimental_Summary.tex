...
Vojtěch Šálek; University of Chemistry and Technology in Prague (UCT)\\
...
Yan Ding; China University of Geoscience \\
...
Loïc Dumazert, Arnaud Regazzi, Rodolphe Sonnier, Laurent Ferry; IMT Mines Alès \\ 
Gilles Parent; Université de Lorraine (LEMTA) \\
...
Björn Beele, Frauke Binder, Adam Slabon, Fabian Brännström; University of Wuppertal \\
Laura Hasburgh, Eleanor Lazarcik; USDA Forest Products Laboratory \\
...
Université de Lorraine (LEMTA) 			& Nancy, France \\ \hline
...
\makecell[l]{Technical Institute of Fire Protection in Prague \\ \& University of Chemistry and Technology, Prague (TIFP+UCT)} & Prague, Czech Republic \\ \hline
...
In TGA experiments, small samples (typically 3~mg to 10~mg) are heated at a constant rate (2~K/min$ \le\beta\le50$~K/min) or maintained at constant elevated temperature in a carefully controlled gaseous environment (either inert or oxidative). Time-resolved measurements of sample mass and temperature are recorded. Care should be taken to ensure that crucibles and thermocouples selected for use in TGA tests are both compatible with the sample and test conditions of interest and that the instrument is calibrated for use under those specific test conditions (i.e., atmosphere and heating rate). Particular attention should be paid to temperature calibration, which should cover the full temperature range of the experiments for each heating rate. Blank or baseline measurements can also be used to correct for apparent changes in sample mass associated with buoyancy and instrumental drift during the heating program. Further detail regarding the principles and practices of TGA is available elsewhere \cite{coats1963thermogravimetric}.
...
Seventeen unique institutions submitted TGA measurements conducted in inert and oxidative gaseous environments. Tests in inert conditions were performed in nitrogen, while oxidative experiments were conducted in oxygen-nitrogen mixtures containing 3, 6, 10, 20, or 21 \% oxygen. Measurements were performed at nine different heating rates ranging from 2~K/min to 50~K/min and, for selected experiments, at two different pressures. One institution also provided TGA data from isothermal tests conducted at 593~K. For each of these experiments, powdered samples between 1~mg and 15~mg were loaded into crucibles, weighed, and then heated from ambient temperature through their primary decomposition reactions at a constant rate. Prior to heating, some samples were held isothermally for approximately 2~min to 30~min to allow the sample and instrument to approach thermal equilibrium and the balance signal to stabilise. In some experiments, this conditioning step was performed at elevated temperature, which may have affected the initial sample moisture content. After heating, some samples were kept under isothermal conditions for approximately 1~min to 30~min.

More information, including apparatus specifications, calibration details, and sample preparation, conditioning, and testing procedures, is provided in the README files for each experiment. This documentation is available on the MaCFP repository \href{https://github.com/MaCFP/matl-db}{https://github.com/MaCFP/matl-db}. Tables~\ref{Table:Matrix_TGA_N2} and \ref{Table:Matrix_TGA_O2} show the number of TGA tests conducted under each combination of test conditions in nitrogen and oxygen-nitrogen atmospheres, respectively. By far the largest interlaboratory dataset was obtained for a heating rate of 10~K/min in nitrogen, for which measurements from 15 institutions are available. Thirteen of these institutions provided at least three replicate measurements, making this condition particularly suitable for assessing interlaboratory variability.
...
\begin{table} [h]
\caption{Test Matrix of TGA Tests Conducted in Oxygen-Nitrogen Atmospheres; table values indicate number of tests conducted under listed test conditions}
\label{Table:Matrix_TGA_O2}
    \begin{center}
        \input{../SCRIPT_FIGURES/TGA/TGA_Oxygen.tex}\\
    \end{center}
\end{table}

\subsection{Differential Scanning Calorimetry (DSC)}
In DSC experiments, a small material sample is placed in a crucible of high thermal conductivity and heated at a constant rate alongside a reference crucible in a carefully controlled gaseous environment. Unlike TGA, sample mass is not continuously measured during the experiment; rather, the initial sample mass is determined prior to the test, while time-resolved measurements of sample temperature and heat flow are recorded. Heat flow to the sample can be measured in two ways: (1) power compensation DSC, in which the energy (supplied by electric current) needed to maintain the sample crucible at the same temperature as the reference crucible is directly measured, or (2) heat flux DSC, in which the temperature difference between the sample and reference pans is carefully measured during heating and prior calibration of the instrument allows for the determination of heat flux as a function of this temperature difference. Further detail regarding the principles and practices of differential scanning calorimetry is available elsewhere \cite{mcnaughton2003differential}.

DSC is primarily used to identify and characterize endothermic and exothermic thermal events, including the temperatures at which they occur and the associated heats of reaction or transition. Under appropriately controlled and calibrated conditions, DSC can also be used to determine the heat capacity of a material. These features of interest can be determined based on the deviations of measured heat flow to the sample versus from the baseline (heat flow to empty crucibles). In DSC tests, the baseline is not necessarily easy to establish. The baseline itself may deviate from zero for a variety of reasons including a mismatch of the thermal properties of the sample and the reference material, poor positioning of crucibles themselves or of the samples within their crucible, and/or asymmetry in the construction of sample and reference holders. Excellent thermal contact between the sample, the reference pans, and the instrument and careful calibration of the instrument for the exact test conditions used in each experiment is essential to obtaining accurate measurements of sample heat capacity or heats of decomposition. In addition to temperature calibration (similar to TGA), DSC also requires calibration of the heat-flow sensitivity, which relates the measured instrument response to the actual heat flow. This sensitivity calibration can be particularly challenging at high heating rates. Both temperature and sensitivity calibrations should cover the relevant temperature ranges and heating rates used in the experiments.

Seven institutions submitted DSC measurements. Six institutions provided measurements conducted in pure nitrogen, while one institution also provided measurements in oxygen-nitrogen atmospheres containing 3, 6, 10, or 21 \% oxygen. All submitted DSC measurements were obtained in Simultaneous Thermal Analysis (STA) mode described below, meaning that sample mass was recorded simultaneously with heat flow. Thus, in addition to the DSC signal, corresponding thermogravimetric data are available for these experiments, which is not inherent to conventional DSC measurements. Across all experiments, measurements were conducted at heating rates between 2.5~K/min and 50~K/min. For most of these experiments, powdered samples between 3~mg and 11~mg were loaded into metallic crucibles, weighed, and then heated from ambient temperature through complete thermal decomposition at a constant heating rate.

More information, including  apparatus specifications, calibration details, and sample preparation, conditioning, and testing procedure, is provided in the README files for each experiment. This documentation is available in the MaCFP repository \href{https://github.com/MaCFP/matl-db}{https://github.com/MaCFP/matl-db}. Tables~\ref{Table:Matrix_DSC_N2} and \ref{Table:Matrix_DSC_O2} show the number of DSC tests conducted under each combination of test conditions in pure nitrogen and oxygen-nitrogen atmospheres, respectively.
...
\begin{table}[ht]
    \caption{Test Matrix of DSC Tests Conducted in Oxygen-Nitrogen Atmospheres; table values indicate number of tests conducted under listed test conditions}
    \label{Table:Matrix_DSC_O2}
    \begin{center}
        \input{../SCRIPT_FIGURES/DSC/DSC_Oxygen.tex}\\
    \end{center}
\end{table}
...
In STA experiments, a small material sample is placed in a crucible of high thermal conductivity and heated at a constant rate alongside a reference crucible in a carefully controlled gaseous environment. In these tests, both sample mass (TGA) heat flow to the sample (DSC) are recorded during a single, combined experiment. In this report, the thermogravimetric signal obtained from STA experiments is combined with data from conventional TGA measurements conducted under corresponding test conditions. The heat-flow signal is treated as DSC data; in the present dataset, all submitted DSC measurements were obtained using STA instruments.
...
Six institutions provided measurement data from repeated MCC tests conducted in five different gaseous environments at three different heating rates. Additionally, two institutions conducted MCC experiments on char (formed in MCC tests under nitrogen atmosphere) in air atmosphere. In each test, powdered samples between 1~mg and 12~mg were loaded into ceramic crucibles, weighed, and then introduced into the MCC pyrolyzer. Often, samples/crucibles are held in the pyrolyzer for 10~min at approximately 350~K (to allow for equilibrium conditions: system temperature and gaseous environment) and then heated from this temperature through complete thermal decomposition at a constant heating rate. Additional information, including  apparatus specifications, calibration details, and sample preparation, conditioning, and testing procedure, is provided in the README files for each experiment, which can be found in the MaCFP repository \href{https://github.com/MaCFP/matl-db}{https://github.com/MaCFP/matl-db}. Tables~\ref{Table:Matrix_MCC_N2}, \ref{Table:Matrix_MCC_O2}  and ~\ref{Table:Matrix_MCC_char} show the number of MCC tests conducted by each institution under each combination of test conditions.
...
\begin{table}[ht]
	\caption{Test Matrix of MCC Tests conducted on char in 20 Vol\% oxygen at a heating rate of 60~K/min; table values indicate number of tests conducted under listed test conditions}
	\label{Table:Matrix_MCC_char}
	\begin{center}
		\input{../SCRIPT_FIGURES/MCC/MCC_Char.tex}\\
	\end{center}
\end{table}
...
\label{g_tests_cone}
In cone calorimetry experiments, gram-scale samples (typically 10~cm square) are placed on a layer of thermal insulation inside of a steel holder and exposed to a conical, radiant heater capable of providing an external heat flux of 10~kW/m$^2$ to 75~kW/m$^2$. Samples can be supported in either the horizontal or vertical configuration (typically horizontal) with or without a spark igniter.  This provides (nominally) a one-dimensional heating scenario where sample slabs can burn freely in well-ventilated, open-atmosphere conditions. Samples are supported above a load cell and positioned beneath a well-instrumented exhaust hood, capable of providing oxygen consumption measurements. Thus, the apparatus produces time-resolved measurements of sample mass loss and heat release rates. Additional test conditions, such as specimen thickness and conditioning, type and thickness of backing insulation, wrapping, or metal frame, may vary between experimental setups and can significantly affect the measured response. These conditions should therefore be clearly documented when reporting cone calorimetry results. Further detail regarding the operating principles, best practices, complete capabilities, and analysis techniques of cone calorimetry is available elsewhere \cite{babrauskas1984development, SFPEHandbookCone, astm1354standard, twilley1988user}.
...
Five institutions performed anaerobic gasification tests on `MaCFP-Wood' using two different experimental apparatuses. Three institutions conducted a total of 36 experiments using Controlled Atmosphere Cone Calorimeters, with samples approximately 25~mm thick. Two institutions conducted a total of 14 experiments using Controlled Atmosphere Pyrolysis Apparatuses (CAPA), with samples approximately 9~mm thick. Both apparatuses provide nominally one-dimensional heating of samples under controlled anaerobic conditions.
...
\begin{table}[ht]
	\caption{Test Matrix of Anaerobic Gasification Experiments; table values indicate the number of tests conducted under the listed test conditions, reported as parallel/perpendicular grain orientations}
...
\textbf{Controlled Atmosphere Cone Calorimetry}\\
...
\textbf{Controlled Atmosphere Pyrolysis Apparatus (CAPA)}\\
...
“Direct measurements” of thermal conductivity were provided by two institutions, while thermal diffusivity measurements were provided by three institutions. Two measurement techniques were used: the Transient Plane Source (TPS) Hot Disk method \cite{gustafsson1991transient} and the Laser Flash method \cite{NeztschLFA}. Thermal conductivity was measured using TPS Hot Disk apparatuses, whereas thermal diffusivity was determined using both TPS Hot Disk and a Netzsch LFA 467 HT Hyperflash. The LFA 467 HT Hyperflash uses a xenon light source and an InSb infrared detector.

Experiments were conducted on virgin wood samples either at room temperature or over a temperature range from 298 K (25 °C) to 573 K (300 °C). Thermal diffusivity measurements were performed for samples with the grain oriented both parallel and perpendicular to the sample surface, allowing the effect of grain orientation on heat transfer through the material to be assessed. A pronounced influence of grain orientation was observed. The reader is referred to the technical reference guides and user manuals of the respective instruments for further information on their operating principles and test procedures.
...
\label{optical_tests}
Measurements of spectrally-dependent sample reflectivity were determined using experimental apparatuses consisting of an FTIR spectrometer coupled with a detector and an integrating sphere. First, a reference measurement is performed in the sphere without a sample (system sealed with the same gold coating), then the flux reflected by the sample is measured, and finally the sample reflectivity is calculated as the ratio between reflected flux and reference flux. Various combinations of sources, detectors, and integrating spheres can be used for sample reflectivity measurements. Here, three institutions provided reflectivity data for virgin wood samples, and two institutions also provided data for charred wood samples. The char samples were obtained from specimens previously exposed in CAPA or cone calorimeter experiments at known incident heat fluxes. Additional information, including experimental set-up and calibration details, is provided in the README file of each experiment.  In these tests, directional-hemispherical reflectivity ($\rho$) is measured at wavelengths between 1.4~$\mu$m and 10~$\mu$m. As wood samples are opaque, their transmissivities are assumed equal to 0 and their absortivities are calculated as the complement of reflectivity to 1: $\alpha=1-\rho$. More information about this experimental technique is available elsewhere~\cite{boulet2012characterization}.


\chapter{Experimental Results}
\label{exp_results}
In this section, experimental measurements from both mg- and g-scale tests are presented. When tests were conducted under the same nominal experimental conditions (e.g., heating rate, incident heat flux, and/or gaseous environment) and when data provided by different institutions showed qualitative agreement, average curves (e.g., heat release rate or mass loss rate vs. time or temperature) were calculated; these mean values are identified throughout the text by symbols with overbars. Type A uncertainties~\cite{taylor1994nist} of these mean values are reported for all data as two standard errors of the mean calculated over at least three independent observations, unless otherwise stated.

It should be noted that some variations between datasets are simply stochastic (i.e., random, unavoidable noise in repeated tests); however, others may result from systematic causes. For mg-scale experiments, these may include differences in temperature and sensitivity calibration, actual heating rate, crucible configuration, purge-gas conditions, sample mass, or initial moisture content. For g-scale experiments, systematic differences may arise from incident heat-flux calibration, spatial non-uniformity of the imposed heat flux, specimen density, moisture content, backing insulation, sample holder or frame configuration, grain orientation, and ignition procedure. Consequently, although clear outliers are not considered when calculating average curves (see further discussion throughout this section on how outliers are identified), care should be taken to understand if/how underlying differences in test conditions or procedure may have impacted the response of samples during experiments and thus how this may affect the final averaged dataset. Ultimately, average curves represent the aggregate of data as received, some of which may require corrections by the original submitting institution (e.g., if a dataset was incorrectly labeled or submitted).

The measurement data presented here reflect the status of the MaCFP database as of September 16, 2026. The analysis builds on the preliminary results presented in June 2026 in La Rochelle, France, where several approaches to interlaboratory comparison were introduced, including assessment of repeatability and reproducibility and evaluation of systematic differences between datasets. Since then, the database has been expanded with additional measurements and contributions from several institutions, and the analyses have been updated accordingly. The data treatment presented in this report reflects the current outcome of the discussions within the MaCFP community and the analyses considered most useful for interpretation of the available dataset. While this report represents the final summary of the present MaCFP-4 experimental analysis, it should not be regarded as defining the full extent of analyses that can be performed using the database. The underlying data may support further or alternative analyses in the future where these are scientifically justified or respond to specific needs of the community. The measurement data remain available in \href{https://github.com/MaCFP/matl-db/tree/master/Wood/Calibration_Data}{the MaCFP repository}.
...
All TGA experiments submitted to this workshop were performed at constant nominal heating rates between 2~K/min and 50~K/min. In practice, the actual heating rate may vary from the prescribed value throughout the course of an experiment, especially during its initial stage while the TGA furnace is approaching the programmed heating rate. Figure~\ref{Fig:TGA_dTdt} shows actual time-resolved heating rates in tests conducted nominally at $\beta$ = 5~K/min, 10~K/min, 20~K/min, and 30~K/min. Each curve represents the mean instantaneous heating rate calculated from repeated experiments conducted by the same institution under the same test conditions. When multiple curves are shown for the same institution, they represent average heating rates measured under different test conditions, such as different gaseous environments (e.g., pure nitrogen or 21~vol.~\% oxygen in nitrogen) or pressures (e.g., 40~Pa and 100~kPa).
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/dTdt_TGA_5Kmin.pdf}
		\label{Fig:TGA_5K_dTdt}
	\end{subfigure}
	\hspace{-0.01\textwidth}
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/dTdt_TGA_10Kmin.pdf}
	...
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/dTdt_TGA_20Kmin.pdf}
		\label{Fig:TGA_20K_dTdt}
	\end{subfigure}
	\hspace{-0.01\textwidth}
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/dTdt_TGA_30Kmin.pdf}
		\label{Fig:TGA_30K_dTdt}
	\end{subfigure}

    \caption{Average reported instantaneous heating rate during TGA experiments conducted at nominal heating rates of $\beta=5$, 10, 20, and 30 K/min. Each curve represents the average of repeated experiments conducted by a single institution under the same test conditions. Repeated curves for the same institution represent measurements conducted under different test conditions, such as gaseous environment or pressure. The discontinuity observed for `Orpington' at $\beta=10$ K/min arises from rapid heating to 373~K followed by an extended isothermal hold at this temperature for sample drying prior to decomposition at the nominal heating rate.}
    \label{Fig:TGA_dTdt}
\end{figure}

As shown in Figure~\ref{Fig:TGA_dTdt}, the actual heating rate can vary significantly during the early stages of testing, typically within approximately the first 50--150~K of temperature increase, before approaching its nominal value. In most cases, this behaviour reflects the dynamic response of the instrument furnace and its temperature-control system and is therefore strongly instrument dependent. Such deviations are difficult to eliminate completely, as they depend on factors such as furnace design, controller settings, and the prescribed temperature program. The effect generally becomes more pronounced at higher nominal heating rates, where the furnace and control system must respond more rapidly to maintain the programmed temperature increase. For a smaller number of tests, more pronounced departures from the nominal heating programme, including temporary overshoots, undershoots, or discontinuities, were observed. Some of these features arise from intentionally programmed steps in the experimental procedure; for example, the discontinuity observed for `Orpington' at 10~K/min results from rapid heating to 373~K followed by an extended isothermal hold for sample drying before the subsequent constant-rate heating stage.

Typically, the heating rate stabilised before substantial sample decomposition occurred. In most cases, the stabilised value was close to the nominal heating rate; however, for some experiments, a persistent offset remained and the prescribed heating rate was not fully reached even after the initial transient period. Deviations from the prescribed heating programme may be particularly important in DSC and STA measurements, where both transient and persistent heating-rate errors can influence the measured heat-flow signal and consequently quantities derived from it, such as heat capacity or heats of reaction. For experiments conducted by a single institution at the same nominal heating rate but under different gaseous environments or pressures, no consistent systematic differences in the time-resolved heating-rate profiles were apparent; rather, the profiles were generally highly repeatable within each institution.

The practical importance of deviations between nominal and actual heating rate depends on the process or property being studied. They may be particularly important for low-temperature phenomena and for quantities directly linked to the temperature programme, such as heat capacity in DSC or STA experiments. If TGA, DSC, or STA data are subsequently used for calibration or parameter optimisation, assuming a constant heating rate where the actual rate varies may introduce systematic errors. The actual heating-rate profile should therefore be checked over the temperature interval of interest and, where necessary, the measured time-dependent heating rate should be used instead of the nominal constant value. This is illustrated further in Figure~\ref{Fig:TGA_dTdt_ranges}, where the approximate temperature ranges associated with different physical and chemical processes are highlighted. At $\beta=5$ K/min, the largest deviations are concentrated mainly within the evaporation range, whereas the heating rate is relatively stable during pyrolysis. At $\beta=30$ K/min, deviations persist over a broader temperature range; the `Rouen' data also suggest a small influence of strongly thermal sample processes on the measured heating rate.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[t]{0.7\textwidth}
        \centering
        \includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/dTdt_TGA_5Kmin_ranges.pdf}
        \label{Fig:TGA_5K_dTdt_ranges}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.7\textwidth}
        \centering
        \includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/dTdt_TGA_30Kmin_ranges.pdf}
        \label{Fig:TGA_30K_dTdt_ranges}
    \end{subfigure}

\caption{Average reported instantaneous heating rate during TGA experiments conducted at nominal heating rates of $\beta=5$ and 30~K/min. Each curve represents the average of repeated experiments conducted by a single institution under the same test conditions. The blue shaded region indicates the temperature interval associated primarily with evaporation, while the yellow shaded region indicates the temperature interval associated primarily with pyrolysis. The highlighted intervals allow the reader to assess whether deviations from the nominal heating rate occur within the temperature range relevant to the process being analysed.}
    \label{Fig:TGA_dTdt_ranges}
\end{figure}
...
Most TGA tests were run at nominally constant heating rates between 2~K/min and 50~K/min; approximately 80~\% of the dynamic tests were conducted at heating rates between 5~K/min and 20~K/min. The largest dataset was obtained at 10~K/min, comprising 72 experiments from 15 institutions across the investigated atmospheres. One institution also conducted tests under isothermal conditions at 593~K. Sample mass and temperature were reported either at regular temperature increments or at fixed time intervals. The most common reporting interval was 0.5~K; less frequently, data were reported at 1~K temperature increments or at fixed time intervals of approximately 1--2.4~s. Reporting frequency was not necessarily constant and, due to the nature of the experiment, measured sample temperature could actually decrease from one time step to the next. For all tests, time, $t$, and normalized mass, $m/m_0$, signals were thus first processed using linear interpolation such that they were each reported at the same regular intervals (i.e., every 0.5~K). 

Normalized mass loss rate was calculated for each replicate $j$ at each temperature, $T_i$, using a central numerical derivative across a 1~K temperature interval, as follows:
\begin{equation}
   \frac{\dot{m}_{i,j}}{m_{0,j}} = \frac{1}{m_{0,j}}\frac{\left(m_{i-1,j}-m_{i+1,j}\right)}{\left(t_{i+1}-t_{i-1}\right)}
\end{equation}
In this way, the time step, $\Delta t$, used for this calculation could vary (see previous discussion on heating rates), while always corresponding to a 1~K temperature interval. Noise in the resulting normalized mass loss rate curves was subsequently reduced by applying a Savitzky--Golay filter (third-order polynomial fit; 41-point window). The same filter was also applied to the normalized mass curves for their subsequent presentation and analysis. These filtering parameters were selected by trial and error and subsequently verified using representative TGA data to ensure that random noise was reduced without significantly affecting the relevant features of the mass-loss-rate curves.

Figure~\ref{Fig:TGA_N2_10K_2} shows the effect of the selected filter on a representative TGA dataset (data submitted by ‘Alabio’ from a test in pure nitrogen at 10~K/min). Filtered and unfiltered normalized mass data are shown on the left axis (green and blue curves, respectively), together with filtered and unfiltered normalized mass loss rate on the right axis (black and red curves, respectively). The selected filter effectively reduces random noise while preserving the relevant features of the signal: individual mass loss events remain clearly resolved, no appreciable shift in their onset or peak temperatures is introduced, and the peak mass loss rate is well preserved.
...
TGA measurements were analyzed to calculate average Mass and MLR curves as well as an estimate of measurement uncertainty for each of these conditions. An effort was made to include as much data as possible in the generation of these average curves; however, clear outliers (full curves, not just individual data points) were not used for the calculation of mean and standard deviation values. Datasets identified as outliers\footnote{Data were excluded from the calculation of average curves for the following reasons: (1) all `Muscovy' data were excluded because measurements were performed using a custom thermogravimetric setup and the actual heating-rate profile differed substantially from the submitted nominal heating rate of 20~K/min; (2) all `Cayuga' data were excluded because the decomposition peak temperature was consistently higher than in the other submissions and the datasets were therefore considered outliers; (3) `Orpington' data below 400~K were excluded because of a sharp initial decrease in mass associated with an isothermal step; and (4) `Pomeranian' data below 380~K were excluded to avoid an initial abrupt decrease in mass loss rate, which produced unrealistically high values of $\dot{m}/m_0$ during the first seconds of the test, particularly at 5~K/min.} are still plotted in Figs.~\ref{Fig:TGA_N2_5K} through \ref{Fig:TGA_O2-21_10K}.% with a note identifying them as outliers in the captions below their respective figures. 

For brevity, this document supplies figures of TGA measurement data only when repeated measurements under those test conditions were supplied by multiple labs; this represents the majority of TGA data received. Normalized mass and mass loss rate curves (individual test data, along with average curves) from all TGA experiments conducted by all labs and under all experimental conditions (all heating rates and gaseous environments) are supplied in Appendix~\ref{App:TGA}.

When repeated measurements were available (submitted by the same or different lab(s) and conducted under the same nominal heating rate and gaseous environment) average mass loss rate was calculated at each temperature, $T_i$, as the mean value reported from all (excluding noted outliers) repeated tests, $j$:
\begin{equation}
\overline{\left(\frac{\dot{m}}{m_0}\right)_i} = \frac{1}{\sum_{j=1}^{N_j}N_i} \sum_{j=1}^{N_j} \sum_{i'=i-n}^{i+n} \frac{\dot{m}_{i',j}}{m_{0,j}}
\end{equation}
where $N_i$ is equal to the number of measurements from an individual experiment, $j$, reported in the interval $(i-n) \leq i' \leq (i+n)$. Since the data were interpolated at 0.5~K intervals, $n=2$ corresponds to a temperature interval of approximately $\pm1$~K around $T_i$. $N_j$ is the number of replicate tests. The standard error of the mean, $\sigma_{mean,i}$, was calculated at each temperature, $T_i$, as:
\begin{equation}
\sigma_{mean,i} = \sqrt{\frac{1}{\left(\sum_{j=1}^{N_j}N_i\right)\left(\sum_{j=1}^{N_j}N_i-1\right)} \sum_{j=1}^{N_j} \sum_{i'=i-n}^{i+n} \left(\frac{\dot{m}_{i',j}}{m_{0,j}} - \overline{\left(\frac{\dot{m}}{m_0}\right)_i}\right)^2}
\end{equation}

\textbf{Mass Loss in Nitrogen}\\
The discussion below focuses primarily on the most extensively tested heating rates in pure nitrogen, while results for the remaining TGA test conditions are provided in Appendix~\ref{App:TGA}. Figures~\ref{Fig:TGA_N2_5K} through \ref{Fig:TGA_N2_20K} show measured mass and mass loss rate data from 19, 56, and 18 tests conducted at nominal heating rates of $\beta$ = 5~K/min, 10~K/min, and 20~K/min, respectively. Across these conditions, the main decomposition stage occurs approximately between 500~K and 700~K and is characterised by one dominant mass-loss-rate peak, although this peak likely represents several overlapping decomposition processes. By 700~K, approximately 75~\% of the initial sample mass has been lost (this represents approximately 90~\% of total mass lost during the test). In all datasets, continued mass loss is observed at higher temperatures (at least up to 1100~K) though measured MLR is much lower in this temperature range. This behavior is consistent with continued carbonization of the residual char formed by lignin decomposition**~\ref{1,2,3}; this behavior is further discussed, later in this section.
...
Increasing the heating rate results in a higher peak normalized mass loss rate and shifts the main decomposition peak towards higher temperatures. Conversely, at lower heating rates, the decomposition occurs over a broader temperature interval and individual overlapping mass-loss processes become more clearly distinguishable. These differences are illustrated in Figure~\ref{Fig:TGA-N2_5_10_20K}, which compares the average mass and mass loss rate curves obtained at nominal heating rates of 2, 5, 10, and 20~K/min. Average curves are shown as solid lines, while the shaded regions represent $\pm 2\sigma_{mean,i}$. Individual experiments are also shown for comparison.
...
\end{subfigure}
	...
	\caption{Average normalized (a) mass and (b) mass loss rate of TGA tests conducted in pure nitrogen at $\beta$ = 2~K/min, 5~K/min, 10~K/min, and 20~K/min. Average curves are calculated from the submitted data under the same test conditions, excluding the data noted above; shaded areas represent $\pm 2\sigma_{mean,i}$.}
 \label{Fig:TGA-N2_5_10_20K}
\end{figure}

Despite the generally similar main decomposition behaviour, two regions of the TGA curves show substantial variability between participating institutions, as highlighted in Figure~\ref{Fig:TGA_N2_10K_marked}, and are therefore considered separately below. The first is the initial mass loss at relatively low temperatures, which is associated with differences in the apparent initial moisture content of the samples. The second is the amount of solid residue remaining after the main decomposition stage and its subsequent evolution at higher temperatures.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/TGA_Wood_N2_10K_Mass_marked_regions.pdf}
		\caption{The highlighted regions indicate the low-temperature mass loss associated with apparent initial moisture content (black) and the variability in residue yield immediately after the main decomposition stage (blue) and at higher temperatures (red).}
		\label{Fig:TGA_N2_10K_marked}
	\end{subfigure}
%	\hspace{-0.01\textwidth}
	\begin{subfigure}[t]{0.49\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/TGA_Wood_N2_10K_Mass_zoom1_marked.pdf}
		\caption{Focus on low-temperature changes in sample mass due to variations in initial sample moisture content. The dashed vertical line at 400~K indicates the temperature used to define the dry reference mass, $m_{400}$. }		\label{Fig:TGA_N2_10K_moisture}
	\end{subfigure}
	\caption{Key variations in TGA data from different institutions (Nitrogen atmosphere, $\beta=10$~K/min)}
	\label{Fig:TGA_N2_10K_marked-moisture}
\end{figure}


\textbf{Accounting for Initial Sample Moisture Content (Dry-Mass Normalization)}\\
Due to variations in sample prep and/or testing conditions, initial mass varied at low temperatures between measurements obtained by different laboratories. Apparent moisture content was therefore calculated as $1-m_{400}/m_0$, where $m_{400}$ is equal to measured sample mass at $T=400$~K. This value was then used to re-normalize sample mass, as sen in Fig.~\ref{Fig:TGA_N2_10K_drynorm}. Apparent moisture contents (MC) from each dataset are reported in Table~\ref{tab:wood_tga_results}, ranging between $0<8.2~$\%. This variability is also evident in the enlarged low-temperature view shown in Figure~\ref{Fig:TGA_N2_10K_moisture}. These differences do not necessarily represent differences in the original material itself, but are strongly affected by sample conditioning and the experimental procedure prior to the main heating stage. Some institutions dried or otherwise conditioned the samples before testing, while others tested the material without additional drying. In some datasets, the initial part of the measurement was also removed before the data were submitted. Consequently, the apparent initial moisture content should be interpreted with caution when comparing results between laboratories.


One approach to reduce this variability would be to prescribe an identical drying and conditioning procedure for all participating laboratories. In practice, however, this would be difficult to implement consistently because of differences in available instrumentation and established measurement procedures. Alternatively, the temperature programme could include an initial heating and drying stage followed by cooling and a second heating ramp, or an isothermal drying step before the main heating ramp, so that subsequent measurements start from a nominally dry sample. Such procedures may be appropriate when moisture itself is of interest or when moisture-sensitive quantities, such as heat capacity, are evaluated. For analysis focused primarily on the subsequent thermal decomposition, however, an additional experimental drying step is not necessary, since the data can instead be rescaled to the dry sample mass during post-processing as illustrated in Figure~\ref{Fig:TGA_N2_10K_drynorm}.
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/TGA_Wood_N2_10K_Mass_before.pdf}
        \label{Fig:TGA_N2_10K_drynorm_before}
    \end{subfigure}
    \hspace{-0.01\textwidth}
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth, trim=6 6 6 6, clip]{../SCRIPT_FIGURES/TGA/TGA_Wood_N2_10K_Mass_400Knorm_after.pdf}
        \label{Fig:TGA_N2_10K_drynorm_after}
    \end{subfigure}

    \caption{Effect of dry-mass normalization on TGA measurements conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min. The left panel shows the original mass curves normalized by the initial sample mass, $m_0$, while the right panel shows the same data after rescaling by the mass measured at 400~K, $m_{400}$, selected as the dry reference mass.}

    \label{Fig:TGA_N2_10K_drynorm}
\end{figure}

In the present analysis, the mass measured at 400~K, $m_{400}$, was selected as the reference dry mass. At this temperature, sample mass remained approximately constant after initial moisture loss and the main pyrolysis stage had not yet begun. Normalized mass is then calculated as $\frac{m}{m_{400}}$ (rather than as typically reported, $\frac{m}{m_{0}}$, where ${m_{0}}$ is initial sample mass); consequently, re-normalized mass at temperatures at $T<400$~K may exceed unity. Although this re-normalization greatly reduces differences between datasets prior to the onset of pyrolysis, it only partially improves the variability\footnote{\textcolor{red}{Consider measured residue yields calculated at both 700~K and 950~K from repeat measurements submitted by all institutions: at both temperatures, the standard deviation of $\frac{m}{m_{400}}$ is \# versus \# for $\frac{m}{m_{0}}$.}} in residue yield measured after the main decomposition stage (see Table~\ref{tab:wood_tga_results}). Across the investigated datasets, this correction increased the calculated residue yield by up to 2 percentage points at 700~K and 1.3 percentage points at 950~K. This demonstrates that differences in the measured residue yield cannot be explained by initial moisture content alone.

\textbf{Char Yield}\\ 
 Char (residue) yield is one of the key quantities derived from TGA measurements. Although it is often defined as a constant value (input parameter) in pyrolysis models, char is not a primary product~\cite{lewellen1977cellulose} and it is formed by competing mechanisms~\cite{broido1975char}, therefore residue yield is a function of decomposition conditions (e.g., heating rate, volatile residence time, gaseous environment, final temperature)~\cite{anca2016reaction, collard2014review,neves2011characterization}. In the present analysis, residue yields were therefore evaluated at both 700~K, shortly after the main decomposition stage, and 950~K, close to the end of the temperature program used by most participating laboratories. As shown in Table~\ref{tab:wood_tga_results}, residue yields at 700~K range from approximately 15~\% to 30~\% of the initial sample mass, whereas the corresponding values at 950~K range from approximately 7~\% to 20~\%. For most datasets, the residue yield decreases by approximately 5.5--7 percentage points between 700~K and 950~K. At these temperatures, hemicellulose and cellulose have largely decomposed, while continued mass loss can be associated with the much broader and slower decomposition of lignin, including secondary pyrolysis at elevated temperatures~\cite{yang2007characteristics}.
 
 Measurements from some laboratories demonstrated further mass loss of up to 10--16 percentage points at temperatures above 700~K (with an apparent second peak MLR around $T=875$~K). This suggests a potential secondary decomposition mechanism: oxidation of the residual char. The presence of a secondary reaction was further investigated by `Call', who performed additional tests specifically designed to ensure anaerobic conditions. In these tests, the purge-gas flow rate (nitrogen) was increased, the measurement chamber was carefully evacuated before each test, and the potential for backflow of oxygen into the system was eliminated. As seen in, Fig.\ref{Fig:TGA_pressure} this eliminated the secondary oxidation step. This confirms that the pronounced high-temperature mass loss observed in some datasets can result from oxygen entering or remaining in the nominally inert test environment. When such a secondary reaction is observed, the presence of oxygen should therefore be carefully considered. Its possible sources include insufficient evacuation of the system, short purge times before measurement, leaks or permeable tubing, low purge-gas flow rates allowing backflow, or contamination of the purge gas (source and/or transfer lines).

\begin{figure}
	\centering
	\includegraphics[width=0.85\linewidth]{../SCRIPT_FIGURES/TGA/TGA_Pressure.pdf}
\caption{Measured mass loss in TGA experiments at low and high pressure, with additional considerations for trace amounts of oxygen in the system (tests conducted in nitrogen, $\beta=10$~K/min). Compared to the 'original' 100~kPa measurements, 'additional' tests were conducted with modifications to the experimental procedure designed to fully eliminate the presence of oxygen in the system.}
	\label{Fig:TGA_pressure}
\end{figure}

\textbf{Atmospheric Pressure}\\
The measurements by `Call' shown in Figure~\ref{Fig:TGA_pressure} also provide an opportunity to examine the effect of pressure. In addition to the atmospheric-pressure measurements discussed above, this institution performed tests at 40~Pa under the same nominal heating rate of 10~K/min. The reduced-pressure measurements show a substantially lower residue yield immediately after the main decomposition stage compared with the 100~kPa measurements, suggesting that pressure may influence the amount of solid residue formed during pyrolysis. However, interpretation of the original pressure comparison is complicated by the residual-oxygen effects identified above. The additional 100~kPa measurements performed with improved oxygen control demonstrate that the pronounced high-temperature mass loss in the original atmospheric-pressure dataset was not caused by pressure itself. Consequently, while the results suggest an effect of pressure on residue formation, the available data from a single institution are insufficient to isolate or quantify this effect independently of other experimental conditions.

\textbf{Oxidation}\\ 
TGA measurements were also conducted in oxidative atmospheres containing between 3 and 21~\% oxygen. These conditions were investigated less extensively than those in pure nitrogen because char oxidation was not a primary focus of MaCFP-4, although it will be considered in future workshops. Figure~\ref{Fig:TGA_O2-21_10K} shows measurements from 4 repeated tests conducted in mixtures of oxygen and nitrogen at approximately atmospheric oxygen concentration ($X_{\rm O_2}=0.20$--0.21) and a nominal heating rate of $\beta=10$~K/min. In these tests, sample mass loss occurs in two distinct stages, with approximately 70~\% of the initial sample mass lost by $T=600$~K and most of the remaining mass lost between approximately $T=750$~K and $T=775$~K as a result of char oxidation.
...
\end{subfigure}
	\caption{Normalized (a) mass and (b) mass loss rate of TGA tests conducted in oxidative atmospheres at a nominal heating rate of $\beta=10$~K/min (Cayuga', $X_{\rm O_2}=0.20$; Hardhead', Mallard', and Alabio', $X_{\rm O_2}=0.21$).}
  \label{Fig:TGA_O2-21_10K}
\end{figure}

\clearpage

\textbf{Repeatability and Reproducibility}\\
Figure~\ref{Fig:TGA-peakMLR-N2_10K} summarises the peak normalized mass loss rate and the corresponding peak temperature obtained from TGA tests conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min. This condition was selected for the main-text comparison because it contains the largest number of measurements and participating laboratories; corresponding results obtained at 5 and 20~K/min are provided in Appendix~\ref{App:TGA}. The intralaboratory repeatability of the peak temperature was typically within approximately 5~K, while the interlaboratory range was approximately 10~K when the identified outlier (`Cayuga') was excluded. For peak normalized mass loss rate, the intralaboratory variability was generally below $1\times10^{-4}$~s$^{-1}$, although values approaching $2\times10^{-4}$~s$^{-1}$ were observed for a small number of laboratories. The maximum relative intralaboratory variability was below approximately 15~\%. The interlaboratory range was approximately $4\times10^{-4}$~s$^{-1}$, corresponding to about 30~\%.

Compared with the MaCFP-2 measurements on PMMA~\cite{PreliminarySummary}, summarised in Table~\ref{tab:tga_macfp2_comparison}, the present MaCFP-4 results show slightly better interlaboratory reproducibility in peak temperature: the interlaboratory range decreased from approximately 15~K for MaCFP-2 to approximately 10~K for MaCFP-4, while the intralaboratory range remained approximately 5~K in both studies. In contrast, the relative variability of peak MLR is somewhat larger in MaCFP-4, both within individual laboratories and between laboratories, despite the absolute variability remaining of a similar magnitude. This difference reflects the substantially greater heterogeneity of the wood samples used in MaCFP-4 compared with the relatively homogeneous PMMA material investigated in MaCFP-2. The wider range of sample masses used in the present study, approximately 1--15~mg, may also contribute to the observed variability; however, the available data do not allow this effect to be isolated from other interlaboratory differences.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.85\linewidth]{../SCRIPT_FIGURES/TGA/Tpeak_Average_N2_10K_MLR.pdf}
	\caption{Peak normalized mass loss rate and corresponding peak temperature obtained from TGA tests conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min.}
	\label{Fig:TGA-peakMLR-N2_10K}
\end{figure}

\begin{table}[htbp]
    \centering
    \caption{Approximate comparison of TGA repeatability and reproducibility observed in MaCFP-4 (wood) and MaCFP-2 (PMMA).}
    \label{tab:tga_macfp2_comparison}
    \input{../SCRIPT_FIGURES/TGA/TGA_MaCFP2_Comparison.tex}
\end{table}
...
\item \textbf{Apparent moisture content, MC}: The apparent moisture content was defined as $1-m_{400}/m_0$, where $m_{400}$ is the sample mass at 400~K and $m_0$ is the initial sample mass. A negative value indicates that the recorded mass at 400~K is higher than the initial mass.
\item \textbf{Char yield}: Char yields were evaluated at 700~K and 950~K. Values are reported relative to both the initial sample mass, $m_0$, and the dry reference mass at 400~K, $m_{400}$. The value at 700~K represents the char yield shortly after the main decomposition stage, whereas the value at 950~K represents the char remaining near the end of the temperature programme used by most participating laboratories. Differences between these values reflect continued high-temperature mass loss, which may include both slow decomposition of the char and, in some datasets, oxidation caused by residual oxygen, as discussed above.
\end{itemize}

The arithmetic mean and standard deviation of each quantity are reported in Table~\ref{tab:wood_tga_results} for the different experimental conditions. Where repeated measurements were available, the reported uncertainty represents one standard deviation of the measurements performed by the same institution under the same test conditions.

	\begin{table}[htbp]
		\centering
		\caption{Summary of values of interest derived from the TGA measurements. Reported quantities include peak normalized mass loss rate, temperature of peak mass loss rate, onset temperature, apparent moisture content, and residue yield measured at 700~K and 950~K for experiments conducted under different heating rates and atmospheres. Residue yields are reported relative to both the initial sample mass, $m_0$, and the dry reference mass at 400~K, $m_{400}$.}
...
A total of 59 DSC datasets were submitted by seven institutions. All DSC measurements were obtained as part of simultaneous thermal analysis (STA), with thermogravimetric and heat-flow data recorded during the same experiment.\footnote{Conventional DSC does not measure sample mass and is therefore best suited to quantitative evaluation of thermal events occurring without substantial mass change. When decomposition occurs, changes in sample mass can affect the DSC baseline and quantities normalised by sample mass. Simultaneous TGA-DSC measurements partly overcome this limitation by providing the corresponding mass-loss history, although STA generally represents a compromise compared with dedicated DSC measurements optimised specifically for calorimetric accuracy.} Of these measurements, 41 tests were conducted in pure nitrogen at nominal heating rates between 3 and 50~K/min, with the largest number of 16 measurements obtained at 10~K/min. A further 18 tests were conducted at various heating rates in oxidative atmospheres containing 3--21~\% oxygen in nitrogen.

Although most experiments were conducted using nominally constant heating rates, the actual sample heating rate, $dT/dt$, was not constant throughout every test. The largest deviations generally occurred during the initial part of the heating programme, before the system approached the prescribed heating rate. As discussed for the TGA measurements and illustrated in Figure~\ref{Fig:TGA_dTdt_ranges}, the relevance of these deviations depends on the temperature range of the process being analysed. Consequently, interpretation of DSC signals, particularly at lower temperatures, should consider the measured sample-temperature history rather than the nominal heating rate alone.

\textbf{Data Processing} \\
The key measurements recorded during DSC tests are heat flow to the sample, $\dot{q}/m_0$ (W/g), sample temperature, $T$~(K), and the time, $t$~(s), at which these values were recorded.  Both heat flow $\dot{q}$ and temperature $T$ were reported as time-resolved measurements. Prior to further analysis, the submitted heat-flow measurements were adjusted to a common sign convention such that endothermic events (e.g., heat flow to the sample during thermal decomposition, which peaks at approximately 600~K to 650~K) would be recorded as positive (increasing) events, with respect to the baseline. However, due to significant deviations in reported DSC data (potentially as a result of calibration, baseline correction, or instrumentation issues), negative and/or positive heat flow measurements do not necessarily indicate exothermic and/or endothermic events. Consequently, caution should be exercised when attempting to use these measurements for determination of `MaCFP-Wood' heat capacity, $c_p$, or heat of decomposition.\footnote{Note: `Ancona' DSC data were submitted with the comment, "DSC data serve as supplementary only and are not suitable for calibration." This dataset is thus included here for qualitative comparison and caution should be used if considering it for model calibration/property determination.} 

Most DSC tests were run at constant heating rates. Both heat flow to the sample and sample temperature were reported at regular time or temperature intervals. Much like TGA data, effectively this meant that heat flow to the sample was reported, in most experiments, approximately every 0.5~K. One institution reported data every 1~K, while another reported data at constant time intervals of 1~s. Because the reporting interval was not always defined directly in terms of temperature, and because measured sample temperature could occasionally decrease slightly from one time step to the next, the temperature spacing between consecutive reported data points was not strictly uniform in all datasets.
...
\end{equation}
Note: integral heat flows reported here are simply calculated from beginning to end of each test, thus care should be taken when comparing results submitted by different institutions that may have used different initial sample temperatures.

Due to substantial differences in heat flow measurements submitted by different laboratories under nominally identical heating and gaseous environment conditions, average DSC curves were primarily calculated separately for each institution. When repeated measurements were available from the same institution under the same experimental conditions, average heat flow and integral heat flow were calculated at each temperature point, $T_i$, using all available measurements within a 2~K interval centred at $T_i$:

\begin{equation}
\overline{\left(\frac{\dot{q}}{m_0}\right)_i}=\frac{1}{\sum_{j=1}^{N_j}N_{i,j}}\sum_{j=1}^{N_j}\sum_{i'=i-2}^{i+2}\frac{\dot{q}_{i',j}}{m_{0,j}}
\end{equation}

where $N_{i,j}$ is the number of available measurements from experiment $j$ within the interval $i-2 \leq i' \leq i+2$. This corresponds to a total temperature interval of 2~K around $T_i$. The standard error of the mean, $\sigma_{mean,i}$, was calculated at each temperature, $T_i$, as:

\begin{equation}
\sigma_{mean,i}=\sqrt{\frac{\sum_{j=1}^{N_j}\sum_{i'=i-2}^{i+2}\left(\frac{\dot{q}_{i',j}}{m_{0,j}}-\overline{\left(\frac{\dot{q}}{m_0}\right)_{i'}}\right)^2}{\left(\sum_{j=1}^{N_j}N_{i,j}\right)\left(\sum_{j=1}^{N_j}N_{i,j}-1\right)}}
\end{equation}

The same averaging procedure was applied to integral heat flow. Average curves are shown together with shaded regions representing $\pm 2\sigma_{mean,i}$.

\textbf{Heat Flow in Nitrogen} \\
Figure~\ref{Fig:DSC_N2_10K} shows measurement data from repeated tests conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min; data shown here were received from five unique institutions. Instantaneous and integral heat flow curves from DSC experiments conducted under all nitrogen heating-rate conditions are provided in Appendix~\ref{DSC_N2}. Between laboratories, variation between average curves is substantially lower than between individual test results. It is worth noting that, despite the variability in the submitted results, scatter in these measurements is lower than that observed in MaCFP-2, which studied PMMA, a material with comparatively simpler decomposition behavior~\cite{PreliminarySummary}.

\begin{figure}[p]
\centering

\includegraphics[width=0.85\textwidth]{../SCRIPT_FIGURES/DSC/DSC_Wood_N2_10K_HF.pdf}

\vspace{0.5em}

\includegraphics[width=0.85\textwidth]{../SCRIPT_FIGURES/DSC/DSC_Wood_N2_10K_iHF.pdf}

\caption{Measured heat flow and integral heat flow from DSC tests conducted in pure nitrogen at a nominal heating rate of $\beta=10$~K/min.}
\label{Fig:DSC_N2_10K}
\end{figure}

The comparison in Fig.~\ref{Fig:DSC_N2_10K} also highlights several aspects that should be considered in future DSC measurements. Most of the submitted experiments were primarily designed as thermogravimetric measurements rather than optimised specifically for quantitative DSC analysis. Factors such as crucible material (conductivity), sensor configuration, and baseline correction can therefore contribute to the observed inter-laboratory differences~\cite{NetzschDSCBaseline,Salek2023thesis}. In particular, the submitted datasets do not always provide sufficient information to determine whether a separate baseline measurement was performed for the specific crucible and experimental conditions or whether an instrument-specific baseline correction was applied. Submission of the corresponding baseline measurements or correction procedure would therefore be beneficial in future MaCFP calls.

Sample moisture represents an additional source of uncertainty. The submitted samples were not conditioned to a common moisture content prior to testing, so measurements from different institutions effectively started from different initial material states. Evaporation of moisture produces a pronounced thermal effect at lower temperatures and can substantially disturb the heat-flow signal before the main decomposition stage~\cite{Salek2023thesis}. This complicates identification of the beginning of thermal decomposition and can influence the placement of the linear baseline between the onset and endset temperatures used for the heat-of-decomposition analysis described below. Moreover, heat flow is normalised by the initial sample mass, including moisture that is no longer present during the main decomposition stage; consequently, derived heats of decomposition are not strictly expressed on a dry-material basis. Different initial moisture contents can also directly contribute to differences in integral heat flow between laboratories.

The integral heat flow calculated over the entire experiment is particularly sensitive to these effects, as well as to differences in the initial heat-flow offset, baseline drift, and instrument-specific transients at the beginning of the heating programme. Since even a relatively small offset can accumulate into a substantial difference in integral heat flow over a long experiment, these curves should be interpreted with caution. For quantitative determination of reaction enthalpies, integration of individual thermal events relative to an appropriate local baseline is therefore generally more meaningful than integration of the complete DSC signal.

\textbf{Heat of Decomposition} \\
Due to the deviations between DSC heat flow measurements submitted by different laboratories, the use of these data for direct determination of heat capacity or heat of decomposition requires caution. However, the simultaneous measurement of sample mass and heat flow in the STA experiments allows an estimate of the heat of decomposition of MaCFP-Wood to be obtained.

First, onset and endset temperatures of the primary mass loss reaction were defined (following the approach in Section~\ref{TGA_analysis}) as the lowest and highest temperatures, respectively, at which normalized mass loss rate exceeded 10~\% of the peak value. The onset temperature was subsequently adjusted, where necessary, to ensure that the linear heat-flow baseline between the onset and endset remained below the measured heat-flow curve up to the peak. On average, approximately 70~\% of initial sample mass was lost as samples were heated through this temperature range. Normalized sample mass loss due to decomposition across this temperature range was calculated:

\begin{equation}
\frac{\Delta m}{m_0}=\frac{m_{\rm onset}}{m_0}-\frac{m_{\rm endset}}{m_0}
\end{equation}

Next, a linear baseline of heat flow to the sample crucible (neglecting the energy needed for decomposition) was defined between the onset and endset heat-flow values. The heat flow above this baseline was integrated over the corresponding time interval. Subtraction of this baseline from the measured heat flow during the period of active decomposition and scaling of the integrated value by measured normalized mass loss in this period produces an estimate of the total heat of decomposition of this reaction:

\begin{equation}
h_{\rm r}=\frac{\displaystyle\int_{t_{\rm onset}}^{t_{\rm endset}}\left[\left(\frac{\dot{q}}{m_0}\right)-\left(\frac{\dot{q}}{m_0}\right)_{\rm baseline}\right]dt}{\Delta m/m_0}
\end{equation}
Individual values of $h_{\rm r}$ are presented in Table~\ref{Table_11} for each individual STA test conducted in a nitrogen environment.\footnote{The `Ancona' DSC data were submitted as supplementary information to the corresponding TGA measurements and were not acquired with the aim of maximising DSC measurement accuracy. These values should therefore be interpreted with caution in quantitative comparisons.} Despite significant scatter in heat flow measurements from individual tests conducted by different labs (see Fig.~\ref{Fig:DSC_N2_10K}), the calculated values of $h_{\rm r}$ have a mean of 369~J/g and a standard deviation of 134~J/g.

\begin{table}[h]
\caption{Calculated heats of decomposition, $h_{\rm r}$ (J/g), for individual STA tests conducted in nitrogen, including mean $\pm$ standard deviation for each institution and overall.}
\label{Table_11}
\begin{center}
...
\textbf{Heat Flow in Oxidative Atmospheres} \\
Only 18 DSC datasets were submitted for oxidative atmospheres, all from a single institution, covering oxygen concentrations between 3 and 21~\% in nitrogen and nominal heating rates between 2.5 and 20~K/min. These data are therefore not suitable for inter-laboratory comparison, and oxidative DSC behaviour was not a primary focus of the present MaCFP call. For this reason, only a representative example is discussed here.

Figure~\ref{Fig:DSC_O2-21_10K} shows heat-flow and integral heat-flow curves measured in an oxidative atmosphere at $\beta=10$~K/min; additional oxidative DSC results are provided in Appendix~\ref{DSC_O2}. Compared with the nitrogen measurements, the DSC response under oxidative conditions exhibits a much stronger exothermic contribution divided into two peaks, which is consistent with oxidation of pyrolysis products and residual char. These measurements are therefore included primarily for qualitative illustration of the change in thermal behaviour in the presence of oxygen.

\begin{figure}[p]
\centering

\includegraphics[width=0.85\textwidth]{../SCRIPT_FIGURES/DSC/DSC_Wood_O2-21_10K_HF.pdf}

\vspace{0.5em}

\includegraphics[width=0.85\textwidth]{../SCRIPT_FIGURES/DSC/DSC_Wood_O2-21_10K_iHF.pdf}
\caption{Measured heat flow and integral heat flow from DSC tests conducted in an oxidative atmosphere ($X_{\rm O_2}=0.21$) at a nominal heating rate of $\beta=10$~K/min.}
\label{Fig:DSC_O2-21_10K}
\end{figure}

\newpage

\textbf{Specific Heat Capacity and TM-DSC} \\
Determination of specific heat capacity from DSC measurements requires more careful experimental control than a qualitative comparison of heat-flow curves. Accurate determination of $c_p$ depends on the instrument type and calibration, measurement procedure, sample mass and preparation, crucible characteristics, and reproducibility of the thermal contact between the sample, crucible, and sensor~\cite{mcnaughton2003differential,Salek2024}. For quantitative measurements, a three-run procedure is commonly employed, consisting of a blank measurement, measurement of a reference material with known specific heat capacity, and measurement of the sample under the same temperature programme; synthetic sapphire is commonly used as the reference material~\cite{mcnaughton2003differential,Salek2024}.

A fundamental limitation is that the DSC signal can only be interpreted directly as specific heat capacity when no other significant endothermic or exothermic process occurs simultaneously. If moisture evaporation, thermal decomposition, curing, oxidation, or another thermal event occurs, the measured response represents an apparent heat capacity containing contributions from both sensible heating and the corresponding reaction or phase-change enthalpy. For this reason, dry samples and a temperature range below the onset of thermal decomposition are preferable for direct $c_p$ determination~\cite{Salek2024}.

Specific heating programmes are therefore often used for $c_p$ determination. Besides conventional three-run procedures, temperature-increment methods~\cite{Salek2024} and modulated DSC approaches can be used to improve separation of heat-capacity-related effects from other thermal processes~\cite{mcnaughton2003differential}. In temperature-modulated DSC (TM-DSC), a periodic temperature modulation is superimposed on the underlying heating programme, allowing the measured heat-flow signal to be separated into reversing and non-reversing components. The reversing component is primarily associated with heat-capacity-related behaviour, whereas non-reversing contributions include kinetically controlled thermal events.

TM-DSC measurements were conducted by one institution (`Mallard') using a custom temperature programme. Samples were heated and cooled three times between 190~K and 430~K in a pure nitrogen environment, with an underlying heating rate of 3~K/min. Heat-flow measurements from the heating phases are shown in Figure~\ref{Fig:UMET_DSC_heatflow}. A pronounced feature is observed below approximately 360~K during the first heating cycle, particularly in the total and non-reversing heat-flow signals. This contribution is consistent with moisture loss from the sample and is strongly reduced in the subsequent heating cycles after the material has been dried during the first cycle.
...
\textbf{**HRR in oxygen containing atmoosphere**}
...
\begin{figure}[h]
	\centering
	\begin{subfigure}[b]{0.75\textwidth}
		\includegraphics[width=1\linewidth]{../SCRIPT_FIGURES/MCC/MCC_PS_N2_60K_HRR.pdf}
		\caption{}
		\label{Fig:MCC_PS_N2_60K_HRR}
	\end{subfigure}
	
	\begin{subfigure}[b]{0.75\textwidth}
		\includegraphics[width=1\linewidth]{../SCRIPT_FIGURES/MCC/MCC_PS_N2_60K_int_HRR.pdf}
		\caption{}
		\label{Fig:MCC_PS_N2_60K_int_HRR}
	\end{subfigure}
	
	\caption{Measured (a) HRR and (b) integral HRR in MCC experiments conducted at 60 K/min under inert atmosphere. The R8 measurement from `Saxony' is identified as outlier and will not be used for the calculation of mean and standard deviation values (peak value too high; potential issue in reported initial sample mass).}
	\label{Fig:MCC_PS_N2_60K}
\end{figure}
...
\begin{figure}[h]
	\centering
	\begin{subfigure}[b]{0.75\textwidth}
		\includegraphics[width=1\linewidth]{../SCRIPT_FIGURES/MCC/MCC_Wood-char_O2-20_60K_HRR.pdf}
		\caption{}
		\label{Fig:MCC_wood-char_N2_60K_HRR}
	\end{subfigure}
	
	\begin{subfigure}[b]{0.75\textwidth}
		\includegraphics[width=1\linewidth]{../SCRIPT_FIGURES/MCC/MCC_Wood-char_O2-20_60K_int_HRR.pdf}
		\caption{}
		\label{Fig:MCC_wood-char_N2_60K_int_HRR}
	\end{subfigure}
	
	\caption{Measured (a) HRR and (b) integral HRR in MCC experiments conducted at 60 K/min under inert atmosphere. The R8 measurement from `Saxony' is identified as outlier and will not be used for the calculation of mean and standard deviation values (peak value too high; potential issue in reported initial sample mass).}
	\label{Fig:MCC_wood-char_N2_60K}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=5.5in]{../SCRIPT_FIGURES/Cone/Gasification_Wood_40kW_MLR.pdf}
...
Thermal conductivity measurements were conducted by two institutions (Mallard and Hardhead) using the Transient Plane Source (TPS) method at approximately 303~K. Each institution performed two repeated measurements. Mallard used a Hot Disk TPS~2500S with a Kapton~8563 sensor (radius 9.868~mm) and a measurement time of 160~s. Samples were conditioned at room temperature (approximately 23~$^\circ$C) and 50~\% relative humidity; the corresponding sample moisture content was not reported. Hardhead used a Hot Disk TPS~1500 with a Kapton~5501 sensor (radius 6.403~mm) and an output power of 50~mW. Measurement times of 160 and 80~s were used. Samples were conditioned at approximately 23~$^\circ$C and 50~\% relative humidity, with a measured moisture content of $10.2 \pm 0.3$~\%.

The individual thermal-conductivity measurements and corresponding mean values are summarised in Table~\ref{tab:TPS_conductivity}. Despite differences in instrumentation and measurement conditions, the mean values reported by the two institutions differ by approximately 5~\%.

\begin{table}[h]
\centering
\caption{Thermal conductivity of MaCFP-Wood measured using the Transient Plane Source (TPS) method. Mean values are reported as mean $\pm$ standard deviation of two repeated measurements.}
\label{tab:TPS_conductivity}
\begin{tabular}{lccccc}
\hline
\textbf{Institution} & \textbf{Apparatus} & \textbf{Moisture content} & \textbf{Temperature} & \textbf{$k_1$ / $k_2$} & \textbf{Mean $k$} \\
& & \textbf{(\%)} & \textbf{(K)} & \textbf{(W/(m\,K))} & \textbf{(W/(m\,K))} \\
\hline
Mallard & TPS 2500S Hot Disk & NA & 303 & 0.147 / 0.144 & $0.1455 \pm 0.0021$ \\
Hardhead & TPS 1500 Hot Disk & $10.2 \pm 0.3$ & 303 & 0.142 / 0.134 & $0.1380 \pm 0.0057$ \\
\hline
\end{tabular}
\end{table}
...
Thermal diffusivity was determined using both the Transient Plane Source (TPS) and Laser Flash Analysis (LFA) methods. TPS measurements were obtained by Mallard and Hardhead using the same experiments described above for thermal conductivity. Mean diffusivities calculated from two repeated measurements were $0.380 \pm 0.039$~mm$^2$/s for Mallard and $0.331 \pm 0.027$~mm$^2$/s for Hardhead, where the uncertainty represents one standard deviation.

Additional thermal diffusivity measurements were conducted by Mallard and Rouen using Netzsch LFA~467 instruments. Mallard performed two measurements at approximately 293~K, both giving a diffusivity of 0.195~mm$^2$/s with a reported result variability of $\pm 0.005$~mm$^2$/s. Rouen used a Netzsch LFA~467 HT HyperFlash with graphite-coated samples tested under nitrogen. Six samples were tested for each grain orientation over the temperature range reported in Table~\ref{tab:Diffusivity}; the tabulated values represent mean values and one standard deviation.

The Rouen measurements demonstrate a strong effect of grain orientation. Samples with the grain perpendicular to the exposed surface exhibited thermal diffusivities approximately two to three times higher than samples with the grain parallel to the surface. Diffusivity in the perpendicular orientation decreased with increasing temperature above approximately 323~K, whereas the parallel-orientation values remained comparatively constant. The measurements also indicate substantial differences between TPS and LFA results obtained near ambient temperature, although direct comparison is complicated by differences in measurement method, sample geometry, preparation, conditioning, and optical coating of the LFA samples.

\begin{table}[h]
\centering
\caption{Thermal diffusivity of MaCFP-Wood measured using the Transient Plane Source (TPS) and Laser Flash Analysis (LFA) methods. TPS values are reported as mean $\pm$ standard deviation of two repeated measurements. For the Mallard LFA measurements, the reported uncertainty represents result variability; Rouen values are mean $\pm$ standard deviation of six samples for each grain orientation.}
\label{tab:Diffusivity}
\begin{tabular}{llccc}
\hline
\textbf{Institution} & \textbf{Method / Apparatus} & \textbf{Temperature} & \textbf{Perpendicular} & \textbf{Parallel} \\
& & \textbf{(K)} & \textbf{(mm$^2$/s)} & \textbf{(mm$^2$/s)} \\
\hline
Mallard & TPS 2500S Hot Disk & 303 & - & $0.380 \pm 0.039$ \\
Hardhead & TPS 1500 Hot Disk & 303 & - & $0.331 \pm 0.027$ \\
\hline
Mallard & Netzsch LFA 467 & 293 & - & $0.195 \pm 0.005$ \\
\hline
Rouen & Netzsch LFA 467 HT & 298 & $0.43 \pm 0.04$ & $0.14 \pm 0.01$ \\
...
\begin{table}[h]
    ...
    \begin{tabular}{l c c c c c}
        ...
        \hline
        Virgin & 48.77 $\pm$ 1.95 & 6.13 $\pm$ 0.25 & 0.05 $\pm$ 0.01 & 0 & 40.69 $\pm$ 2.81 \\
        Char   & 91.19 $\pm$ 3.93 & 2.28 $\pm$ 0.09 & 0.13 $\pm$ 0.01 & 0 & 3.65 $\pm$ 2.74  \\
        Gaseous Volatiles & 31.90 $\pm$ 3.80 & 7.12 $\pm$ 0.43 & 0.00 $\pm$ 0.02 & 0 & 56.18 $\pm$ 4.67 \\
	\hline
    \end{tabular}
...
Spectral reflectivity measurements were submitted by three institutions for virgin `MaCFP Wood' and corresponding char samples. Figure~\ref{Fig:Reflectivity} shows the average measured reflectivity as a function of wavenumber. For virgin wood, measurements were available from Rouen, Canard, and Bali. Char reflectivity measurements were available from Rouen for samples produced at 20, 40, and 60~kW/m$^2$ in CAPA and from Bali for char produced in a cone calorimeter at 50~kW/m$^2$. Shaded regions represent $\pm 2$ standard errors of the mean of repeated measurements.

Based on the measured spectral reflectivity, an effective broadband emissivity was calculated by weighting the reflectivity with the Planck blackbody spectrum:
...
\noindent where $\rho(\nu)$ is the measured spectral reflectivity and $B(\nu,T)$ is the Planck blackbody radiance:
...
Here, $T$ is absolute temperature, $h$ is Planck's constant ($h = 6.626 \times 10^{-34}$ J$\cdot$s), $c$ is the speed of light ($c = 3.000 \times 10^{8}$ m/s), and $k_B$ is Boltzmann's constant ($k_B = 1.381 \times 10^{-23}$ J/K). Effective broadband emissivities were calculated between 293~K and 453~K for each averaged reflectivity spectrum and are reported in Table~\ref{tab:emissivity}.

\begin{figure}[t!]
\centering
\includegraphics[width=4in]{../SCRIPT_FIGURES/Reflectivity/Wood_Average_reflectivity_all.pdf}
\includegraphics[width=4in]{../SCRIPT_FIGURES/Reflectivity/Wood-char_Average_reflectivity_all.pdf}
\caption{Measured spectral reflectivity of `MaCFP Wood' and corresponding char samples. The upper plot shows average reflectivity of virgin wood, while the lower panel shows char samples produced under different heating conditions. Shaded regions represent $\pm 2$ standard errors of the mean.}
\label{Fig:Reflectivity}
\end{figure}

\begin{table}[b]
\centering
\caption{Calculated effective broadband emissivity of `MaCFP Wood' and corresponding char samples between 293~K and 453~K.}
\input{../SCRIPT_FIGURES/Reflectivity/Reflectivity_Emissivity.tex}
...
%
\appendix
\include{MaCFP-4_Experimental_Summary_SI}
