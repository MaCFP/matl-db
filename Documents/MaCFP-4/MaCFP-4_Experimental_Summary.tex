...
** What about plot of peak MLR and Tmax results?
Compared to MaCFP-2 (PMMA) wood results show as good, or better, variation in T\_peak. Although reported measurements of Peak MLR for 'MaCFP Wood' had approximately the same absolute variability as seen in 'MaCFP PMMA' data~\cite{PreliminarySummary}, relative variability (\%) was approximately twice as great, because Peak MLR was lower in these wood tests.
...
\newpage
\begin{table}[h]
\caption{Calculated heats of reaction, $h_{\rm r}$ (J/g), in anaerobic DSC tests per repetition experiment [**Confirm 1000 W/g value for NIST data**]}
\label{Table_11}
\begin{center}
...
\begin{figure}[h]
	...
	\caption{Measured (a) HRR and (b) integral HRR in MCC experiments conducted at 30 K/min under inert atmosphere. The following dataset is identified as outlier and will not be used for the calculation of mean and standard deviation values: 'Crested' (negative integral HRR at high Temperatures; potential baseline correction issue).}
...
\end{subfigure}
  ...
  \caption{Measured (a) HRR and (b) integral HRR in MCC experiments conducted at 60 K/min under inert atmosphere. The following measurement is identified as outlier and will not be used for the calculation of mean and standard deviation values: measurement R8 from Saxony (peak value too high; potential issue in reported initial sample mass).}
...
Figure~\ref{Fig:MCC_N2_avg} plots average heat release rate and integral heat release rate from anaerobic MCC tests conducted at 30, 45, and 60~K/min on a single set of axes, along with measurements of all individual tests conducted. Shaded error bars surrounding these average curves represent two standard deviations of the mean, $2 \sigma_{mean,i}$. As seen here, increased heating rates do not significantly affect the onset temperature of decomposition but allow for a shift (to higher temperatures) in the apparent temperature that heat is released.


**COMMENT ON: 

"Substantially more MCC data than MaCFP-2 (PMMA)...

It’s worth more closely looking at test factors that can affect these results before using them for property calibration. Specifically, let’s consider sample size, heating rate, energy content, and baseline correction – to see how these affect measured HRR.

"Baseline correction matters, especially 
1. For low HRR materials (wood). 2. Low heating rate <60 k/min. 3. Small sample size. 4. Chemical O2 sensor
"

Increased Oxygen availability: Decomposition occurs across a narrower temperature range. Higher HRR at lower temperatures. Limited affect on measure total heat released / fuel energy content
...
\begin{itemize}
	...
	\item \textbf{HR Total}: Total heat released by combustion of the pyrolysis gases (a measure of the material's heat of combustion, when normalized by initial sample mass). Elevated HR Total values measured oxidation tests demonstrate the significant impact (on total heat release) of oxidation of residual char.**rename to Total HR?**
